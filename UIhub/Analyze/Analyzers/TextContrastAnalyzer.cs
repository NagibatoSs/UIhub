...
{
    public class TextContrastAnalyzer : UIAnalyzer
    {
        private static readonly HashSet<string> TextClasses = new()
        {
            "text", "header", "menuItem", "label"
        };

        public override string Code => "CONTRAST_TEXT";

        public TextContrastAnalyzer(IAnalysisCriteria criteriaService) : base(criteriaService) { }

        public override AnalysisResult Analyze(List<UiElement> elements)
        {
            var criteria = _criteriaService.GetByCode(Code);
            var threshold = criteria?.ThresholdValue ?? 4.5;
            ...
            {
                var contrast = ComputeContrastRatio(el.Ocr.TextColorRgb, el.Ocr.BackgroundColorRgb);
                ...
                if (contrast < threshold)
                {
                    result.Items.Add(new AnalysisItem
                    {
                        ...
                        Message = $"Элемент {ElementClassNames.Translate(el.Class)} имеет контраст текста {contrast:F2} " +
                                  $"(норма ≥ {threshold})"
                    });
                }
            }
        ...
        private List<UiElement> GetTextElements(List<UiElement> elements)
        {
            return elements
                .Where(e => TextClasses.Contains(e.Class)
                    && e.Ocr?.TextColorRgb != null
                    && e.Ocr?.BackgroundColorRgb != null)
                .ToList();
        }
